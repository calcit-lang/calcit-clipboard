...
case "$(uname -s)" in
  Darwin)
    library="dylibs/libclipboard.dylib"
    symbols="$(nm -gU "$library")"
    ;;
  Linux)
    library="dylibs/libclipboard.so"
...
if grep -Eq '[[:space:]]_?(copy|paste|abi_version|edn_version)$' <<<"$symbols"; then
  echo "legacy Rust ABI export is still visible" >&2
  exit 1
fi

echo "verified ${#expected[@]} C-safe calcit-clipboard exports and no legacy Rust ABI symbols in $library"
